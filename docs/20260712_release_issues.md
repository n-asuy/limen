...
- 更新署名鍵は生成済み。公開鍵は tauri.conf.json に埋め込み済み。残りの手動作業:
  1. リポジトリsecretsに2つ登録: `TAURI_SIGNING_PRIVATE_KEY`(生成した秘密鍵のbase64。別途受領) / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`(空パスワードで生成したため空文字)
  2. `plugins.updater.endpoints` の `curino/limen` を R-01 で確定する公開リポジトリ名に合わせる(latest.json は tauri-action が Release 資産として生成する)
- 依存: 署名導入(R-08)と配布先(R-01)が前提。実機の更新往復検証はR-04で最初の署名付きReleaseを2つ以上作った後に可能。
